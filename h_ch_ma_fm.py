...
_, repost_times = _parse_cli_args(sys.argv[2:])
print(f"掲示板投稿時刻: {[_fmt_time(t) for t in repost_times]}")
...
def _process_chara(name, chara, driver, wait, mail_info, report_dict,
                   repost_done_today):
  """1キャラ分の処理: checkmail / re_post（指定時刻が来ていれば）"""
  now = datetime.now()
...
try:
  ...
  while True:
    ...
    if now >= today_end:
      ...
      print(f"開始:{today_start.strftime('%H:%M')} / 終了:{today_end.strftime('%H:%M')} / 掲示板投稿:{[_fmt_time(t) for t in repost_times]}")
      print(f"{'='*50}")
    ...
    print(f"翌日も実行。次回チェック: {next_start.strftime('%Y-%m-%d %H:%M')} ({wait_sec/3600:.1f}時間後)")
    time.sleep(wait_sec)
