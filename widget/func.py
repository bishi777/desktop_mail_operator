...
def _katakana_to_hiragana(s: str) -> str:
    # カタカナ→ひらがな（ーなどの長音は一旦取り除き、記号・空白をトリム）
    s = unicodedata.normalize("NFKC", s)
    s = s.replace("ー", "").replace("ｰ", "").replace("―", "")
    out = []
    for ch in s:
        code = ord(ch)
        # カタカナ基本面
...
def _ja_sort_key(s: str) -> str:
    # ひらがなベースへ正規化 → ロケール依存の比較キーに変換
    base = _katakana_to_hiragana(s or "")
    try:
        # 日本語ロケール（無ければ例外→フォールバック）
        locale.setlocale(locale.LC_COLLATE, "ja_JP.UTF-8")
        return locale.strxfrm(base)
    except Exception:
        # ロケールが使えない環境のフォールバック：NFKCで比較
        return unicodedata.normalize("NFKC", base)

def sort_handles_by_name(driver, wait, timeout=4):
    """
    各ハンドルに切り替えて `.mydata_name` のテキストを取得し、
    五十音順（仮名正規化・ロケール考慮）で並べ替えたハンドルのリストを返す。
    """
    ...
    for h in list(driver.window_handles):
        try:
            ...
            name = ""
        pairs.append(( _ja_sort_key(name), name, h ))
    # 元のタブに戻す（念のため）
    try:
    ...
    # ソート（キー→表示名→ハンドルの順で安定）
    pairs.sort(key=lambda x: (x[0], x[1], x[2]))
    return [h for _,__,h in pairs]
