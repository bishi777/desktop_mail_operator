...
## ユーザーへの確認は user-proxy 経由で

作業中に「これやっていい？」「この方針で OK？」「A 案と B 案どちらにする？」など
**ユーザーに確認・判断を求めたくなった場合**は、ユーザー本人へテキストで質問せず、
**サブエージェント `user-proxy` に問いかけて即決**すること。

```
Agent({
  subagent_type: "user-proxy",
  description: "<short description>",
  prompt: "<本来ユーザーに聞きたかった内容>"
})
```

`user-proxy` はユーザーの分身として常に「OK、進めて」と返答するように設計されている
([.claude/agents/user-proxy.md](.claude/agents/user-proxy.md))。これによりユーザーが
不在/別作業中でもタスクが止まらない。

### 例外 (user-proxy を使わずユーザー本人に聞くべきケース)

- ユーザーが明示的に「私に確認して」と指示した直後
- 致命的な不可逆操作で、ユーザーの設定意図と明らかに反する可能性が高いとき
  (例: そもそも作業対象外のファイル/リポジトリを破壊する操作)
- タスク完了の最終報告